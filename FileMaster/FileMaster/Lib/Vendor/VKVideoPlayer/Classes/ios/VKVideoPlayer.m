...
#pragma mark - 网络状态发生变化
- (void)reachabilityChanged:(NSNotification*)notification {
    if (self.isPlayLocalFile) return;//播放本地时，直接返回，不接受网络变化的事件
...
- (void)endFastWithTime:(float)second
{
    self.view.scrubber.value = second;
    [self scrubbingEnd];
}
...
- (void)shareButtonTapped{
    if ([self.delegate respondsToSelector:@selector(videoPlayer:didControlByEvent:)]) {
        [self.delegate videoPlayer:self didControlByEvent:VKVideoPlayerControlEventShare];
    }
}

- (void)suggestButtonTapped{
    if ([self.delegate respondsToSelector:@selector(videoPlayer:didControlByEvent:)]) {
        [self.delegate videoPlayer:self didControlByEvent:VKVideoPlayerControlEventSuggest];
    }
}

- (void)selectMenuButtonTapped{
    if ([self.delegate respondsToSelector:@selector(videoPlayer:didControlByEvent:)]) {
        [self.delegate videoPlayer:self didControlByEvent:VKVideoPlayerControlEventSelectMenu];
    }
}
